...
export function UploadWorkspace({
  ...
  const upload = async () => {
    ...
    try {
      ...
      setPendingCompletion({
        ...
        totalCount,
        lastFileName:
          results[results.length - 1]?.fileName ?? files[files.length - 1]?.name ?? "",
      });
    } catch (err: any) {
  ...
  const confirmCategory = async (result: FileAnalysisResult) => {
    ...
    try {
      ...
      if (pendingCategoryConfirmationRef.current?.fileName === result.fileName) {
  ...
  useEffect(() => {
    ...
    if (hasPendingCategoryActions || pendingCategoryConfirmationRef.current) {
      return;
    }
    ...
    setCompactAnalysisStatus({
      ...
      currentStatus: pendingCompletion.failedCount > 0 ? "Failed" : "Finished",
      remainingCount: 0,
  ...
  return (
    <div className={showHeading ? "upload-page max-w-3xl mx-auto p-6" : "w-full"}>
      {showHeading && (
      ...
      <main className="upload-content">
        ...
        {detailMode === "compact" && compactAnalysisStatus && (
          <section className="mt-6">
            ...
            <div className="space-y-2 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm">
              <p className="text-gray-600">
                {compactAnalysisStatus.remainingCount} file
                {compactAnalysisStatus.remainingCount === 1 ? "" : "s"} out of{" "}
                {compactAnalysisStatus.totalCount} left to analyze
              </p>
              <p className="truncate text-gray-900">
                {compactAnalysisStatus.currentFileName} ({compactAnalysisStatus.currentStatus.toLowerCase()})
              </p>
            </div>
          </section>
        )}

        {detailMode === "full" && (summary || uploadedFiles.length > 0) && (
          <section className="mt-6">
            <label className="block mb-2 font-medium">Upload Details</label>
                ...
                <table className="w-full text-sm border border-gray-200 rounded">
                  ...
                  <tbody>
                    {uploadedFiles.map((uploadedFile, index) => (
                      <tr key={`${uploadedFile.documentId}-${index}`} className="border-t">
                        <td className="border p-2">{uploadedFile.documentId}</td>
                        <td className="border p-2">{uploadedFile.originalName}</td>
                        <td className="border p-2">{uploadedFile.filename}</td>
                        <td className="border p-2">{uploadedFile.mimeType}</td>
          ...
          <section className="mt-6">
            ...
            <div className="space-y-3">
              {displayedAnalysisResults.map((result, index) => {
                const isActivePrompt = activeCategoryPrompt === result;

                return (
                  <article
                    ref={isActivePrompt ? activePromptRef : null}
                  ...
                  >
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <span className="text-sm text-gray-600">{result.categoryName}</span>
                    </div>
                    ...
                    {result.error ? (
                      ...
                      <>
                        {result.needsNewCategory &&
                          ...
                          isActivePrompt && (
                            <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 p-3">
                              ...
                              <p className="text-sm text-gray-800">{result.prompt}</p>
                              <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                                <label className="sr-only" htmlFor={`category-name-${index}`}>
                                  Category name
                                </label>
                                <input
                        ...
                        {result.categoryStatus && (
                          <p className="mt-3 text-sm text-gray-600">{result.categoryStatus}</p>
                        )}
