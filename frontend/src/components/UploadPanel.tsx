...
export default function UploadPanel({
	...
	description = "Add files to the graph without leaving the current view.",
	className,
}: UploadPanelProps) {
				...
				prev.map((item) =>
					item.fileName === result.fileName
						? {
								...
								isCreatingCategory: false,
								categoryStatus:
									getErrorMessage(error) || "Could not create category.",
							}
						: item,
				),
			);
	...
	const shouldApplyCreatedCategory = (
		...
		return (
			itemSuggestionKey ===
				categoryKey(source.suggestedCategoryName ?? source.categoryName) ||
	...
	return (
		<div className={className}>
			<header className="mb-6">
				<h1 id="upload-modal-title" className="text-2xl font-semibold">
					{title}
				</h1>
			...
			<main className="upload-content">
				<div
					...
					className="flex min-h-65 cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-gray-300 p-10 text-center"
					onClick={() => inputRef.current?.click()}
				>
					<div className="grid h-20 w-20 place-items-center rounded-full bg-gray-100 text-3xl">
						↑
					</div>
				...
				<div className="mt-6">
					<button
						...
						className="min-w-60 rounded-md bg-sky-600 px-5 py-3 text-white hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-60"
					>
						{files.length > 0
				...
				{(summary || uploadedFiles.length > 0) && (
					<section className="mt-6">
						<label className="mb-2 block font-medium">Upload Details</label>
									...
									<table className="w-full text-sm">
										...
										<tbody>
											{uploadedFiles.map((uf, i) => (
												<tr key={`${uf.filename}-${i}`} className="border-t">
				...
				{analysisResults.length > 0 && (
					<section className="mt-6">
						<label className="mb-2 block font-medium">Analysis</label>
